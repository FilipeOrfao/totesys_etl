...
def get_json_from_s3(table: str, s3_client=s3_client, ssm_client=ssm_client) -> list:
    latest_date = get_latest_date_parameter(ssm_client=ssm_client)
    res = s3_client.list_objects_v2(Bucket="test-transformation-bucket-fe")
    if "Contents" in res:
        transformation_files = [
            i["Key"]
            for i in res["Contents"]
            if table in i["Key"] and latest_date in i["Key"]
        ]
    else:
        transformation_files = []
        logging.info(f"There is not new data in {table}")

    res = s3_client.list_objects_v2(Bucket="test-extract-bucket-fe")
    extract_files = [
        i["Key"]
        for i in res["Contents"]
        if table in i["Key"] and latest_date in i["Key"]
    ...
    if extract_files != []:
        for extract_file in extract_files:
            if extract_file.split(".")[0] not in transformation_files:
                logging.info(f"There is new data in {table}")

                for file in extract_files:
                    res = s3_client.get_object(
                        Bucket="test-extract-bucket-fe",
                        Key=file,
                    )

                    list_of_dicts = json.load(res["Body"])
...
def dim_date(ssm_client=ssm_client):
    day_dict = {
        "date_id": [],
        "year": [],
        "month": [],
    ...
    late_date_parameter = get_latest_date_parameter(ssm_client=ssm_client)
    datateime_str = datetime.strptime(late_date_parameter, "%Y-%m-%d %H:%M:%S.%f")

    start_date = datetime(2022, 11, 1)
    end_date = datateime_str
    days_to_create = (end_date - start_date).days

    for i in range(days_to_create + 1):
        ...
        day_dict["quarter"].append(int(date.strftime("%m")) // 4 + 1)
...
def save_parque_to_s3(table_name: str, latest_update: datetime, df: pd.DataFrame):
    if df.empty:
        raise ValueError("Dataframe is empty")
    date_str = datetime.strptime(latest_update, "%Y-%m-%d %H:%M:%S.%f")
    file_name = f"s3://transformation-bucket-sorceress/{table_name}/{date_str.year}-{date_str.month}/{table_name}-{date_str}.parquet"
